...
| 点击 **⏭ 跳过** | 跳到下一阶段并**立即开始计时**（不受「自动进入下一阶段」开关影响） |
| 点击 **Tab**（专注/短休/长休） | 手动切换阶段并**立即开始计时** |
| 点击 **📌 图钉** | 收成迷你悬浮小窗（置顶）；悬停小窗浮出 重置/开始·暂停/跳过 按钮；点时间展开回完整窗口 |
| 拖动迷你小窗**到屏幕边缘** | 贴边吸附，收起成进度细条；悬停细条滑出，拖离边缘取消吸附 |
| 点击 **⚙ 齿轮** | 打开设置（时长 / 自动循环 / Agent 集成 / 关于） |
| 设置抽屉 → **关于 → 检查更新** | 查一次 GitHub 上的最新版本；有新版本给「前往下载」（跳转到 release 页） |
...
平时它**一个字节都不写**，只在真出事时落盘 —— 这条日志是为「界面全死（关闭/最小化/托盘右键都没反应）
但倒计时还在走」那类无法复现的卡死准备的：倒计时还在走说明渲染进程活着，问题只可能在主线程不再泵消息，
而日志会直接点名卡在哪一步。想关掉设 `POMODORO_UI_WATCHDOG=0`。
